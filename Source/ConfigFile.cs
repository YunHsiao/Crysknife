...
namespace UnrealBuildTool
{
	...
	public class ConfigFile
	{
		...
		/// <summary>
		/// Static initializer which is called before any ConfigFile instance is created
		/// </summary>
		...
		private static void ReadIntoSections(string Location, Dictionary<string, ConfigFileSection> Sections, ConfigLineAction DefaultAction)
		{
		...
		/// <param name="DefaultAction">The default action to take when encountering arrays without a '+' prefix</param>
		public static ConfigFile ParseFromCommandLine(string IniText, ConfigLineAction DefaultAction = ConfigLineAction.Set)
		{
			ConfigFile Result = new ConfigFile(DefaultAction);
			...
			foreach (string Setting in SettingLines)
			{
				...
				{
					ConfigFileSection? CurrentSection = null;
					if (!Result.Sections.TryGetValue(SectionName, out CurrentSection))
					{
						CurrentSection = new ConfigFileSection(SectionName);
						Result.Sections.Add(SectionName, CurrentSection);
					}

					if (CurrentSection != null)
					{
						string IniKeyValue = Setting.Substring(Setting.IndexOf(':') + 1);
						TryAddConfigLine(CurrentSection, CurrentRemap, "unknown source file", IniKeyValue, 0, IniKeyValue.Length, DefaultAction, Result.Sections);
					}
				}
			}
